...
/**
 * 异步加载外部脚本（JavaScript 文件）并返回一个 Promise
 */
...
/**
 * 加载外部样式表，如果样式表已加载，则返回缓存的 $link 元素，否则创建新的 $link 元素，并将其追加到 <head> 中。返回的对象具有 remove 方法，用于移除样式表。
 */
...
/**
 * 创建包含指定样式的内联 <style> 元素，将其追加到 <head> 中。返回的对象具有 remove 方法，用于移除内联样式。
 */
