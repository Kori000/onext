...
import { IHeaderMenu } from './config'
...
const AccessibleMenu: Component = () => {
  ...
  const showShow = useDebounceValue(useIsScrollUpAndPageIsOver(600) && hasMetaInfo, 120)
  return (
    <RootPortal>
      <AnimatePresence>
        {showShow && (
          <m.div
            ...
            animate={{ y: 0 }}
            exit={{ y: -20, opacity: 0 }}
            className='fixed left-0 right-0 top-[3rem] z-10 flex justify-center duration-100'
          >
            <ForDesktop />
          </m.div>
        )}
  ...
  return (
    <m.nav
      ...
      className={cn(
        ...
        'group [--spotlight-color:hsl(var(--muted-foreground)_/_0.06)]',
        'duration-200',
    ...
    <div>
      <As
        ...
        className={cn(
          ...
          isActive ? 'text-muted-foreground' : 'hover:text-[hsl(240_3.8%_52%)]',
          isActive ? 'active' : '',
          className
        )}
        ...
        {isActive && (
          <m.span
            className={clsx(
              ...
              'bg-gradient-to-r from-accent-foreground/0 via-accent-foreground/70 to-accent-foreground/0'
            )}
